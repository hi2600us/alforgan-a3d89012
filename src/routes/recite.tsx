...
import { Mic, MicOff, Play, RefreshCcw } from "lucide-react";

export const Route = createFileRoute("/recite")({
  head: () => ({
    meta: [
      { title: "اقرأ · والبرنامج يستمع ويصحّح | وقف الفرقان" },
      { name: "description", content: "برنامج تفاعلي: اختر السورة والآية، اقرأ بصوتك، والبرنامج يستمع لك — إن أخطأت أوقفك وصحّح لك الآية كاملة." },
...
// Normalize Arabic text for comparison: strip tashkeel, tatweel; unify hamza/alef/ya/ta-marbuta
...
function RecitePage() {
  ...
  const [mistakeAt, setMistakeAt] = useState<number | null>(null); // index of expected word
  ...
  const expectedWords = useMemo(() => (ayah ? tokens(ayah.text) : []), [ayah]);
  ...
  const loadAyah = async () => {
    ...
    setMistakeAt(null);
    try {
      const r = await fetch(`https://api.alquran.cloud/v1/ayah/${surahNo}:${ayahNo}/editions/quran-uthmani,ar.alafasy`);
      const j = await r.json();
      const arr = j?.data ?? [];
      const text = arr.find((x: { edition: { identifier: string } }) => x.edition.identifier === "quran-uthmani");
      const audio = arr.find((x: { edition: { identifier: string } }) => x.edition.identifier === "ar.alafasy");
      setAyah({
        text: text?.text ?? "",
        audio: audio?.audio ?? "",
        numberInSurah: text?.numberInSurah ?? ayahNo,
      });
    } catch {
      setErrorMsg("تعذّر تحميل الآية.");
      setStatus("error");
    } finally {
      setLoading(false);
    }
  ...
  const startListening = () => {
    ...
    rec.onerror = (e: { error: string }) => {
      if (e.error !== "aborted" && e.error !== "no-speech") {
        setErrorMsg(`خطأ في التعرّف: ${e.error}`);
        setStatus("error");
      }
      setListening(false);
    };
    ...
    <div dir="rtl" lang="ar" className="min-h-screen bg-background text-foreground">
      ...
      <main className="max-w-4xl mx-auto px-6 py-10 md:py-14">
        ...
        <p className="mt-4 text-base text-foreground/80 leading-relaxed max-w-2xl">
          اختر السورة والآية، ثم اضغط «ابدأ الاستماع» واقرأ بصوتٍ واضح. عند أول خطأ سيوقفك البرنامج ويعيد
          تلاوة الآية كاملة بصوت الشيخ مشاري العفاسي.
        </p>

        <div className="mt-8 grid md:grid-cols-3 gap-4 rounded-xl border border-border bg-card p-5">
          ...
          <div className="grid gap-2">
            <label className="text-xs uppercase tracking-widest text-muted-foreground">إجراءات</label>
            <button
            ...
            >
              <Play size={14} /> استمع للآية
            </button>
          </div>
        </div>

        {/* Ayah display */}
        <div className="mt-6 rounded-xl border border-[color:var(--gold)]/30 bg-[color:var(--sand)] p-6 md:p-8 min-h-[140px]">
          {loading ? (
            <p className="text-muted-foreground text-sm">جارٍ تحميل الآية…</p>
          ) : ayah ? (
            <p
              style={{ fontFamily: "Amiri, serif" }}
              className="text-2xl md:text-4xl leading-loose text-[color:var(--emerald-deep)] text-right"
            >
              {tokens(ayah.text).length === 0 ? ayah.text : null}
              {ayah.text.split(/\s+/).map((word, i) => {
                const spoken = spokenWords[i];
                const exp = expectedWords[i];
                let cls = "";
                if (spoken !== undefined) {
                  if (spoken === exp) cls = "text-[color:var(--emerald-deep)]";
                  else cls = "bg-red-100 text-red-700 rounded px-1";
            ...
            <p className="text-muted-foreground text-sm">اختر سورة وآية لبدء التلاوة.</p>
          )}
        </div>
        ...
        {status === "mistake" && mistakeAt !== null && (
          <div className="mt-6 rounded-xl border border-red-300 bg-red-50 p-5 text-right">
            <p className="font-display text-lg text-red-700">
              توقّف — يبدو أن هناك خطأ عند الكلمة رقم {mistakeAt + 1}.
            </p>
            <p className="text-sm text-red-700/80 mt-1">
              الكلمة الصحيحة: <b>{ayah?.text.split(/\s+/)[mistakeAt]}</b>. استمع الآن للتلاوة الصحيحة كاملة ثم أعد المحاولة.
            </p>
          </div>
        )}
        {status === "done" && (
          <div className="mt-6 rounded-xl border border-[color:var(--emerald-deep)]/30 bg-[color:var(--sand)] p-5">
            <p className="font-display text-lg text-[color:var(--emerald-deep)]">
              أحسنت! تلاوة صحيحة بإذن الله. ✓
            </p>
          </div>
        )}
        {status === "error" && (
        ...
        {transcript && (
          <div className="mt-6 rounded-xl border border-border bg-card p-5">
            ...
            <p className="text-lg leading-loose text-foreground/85" style={{ fontFamily: "Amiri, serif" }}>
              {transcript}
            </p>
          </div>
        )}
        ...
        <p className="mt-10 text-xs text-muted-foreground leading-relaxed">
          ...
          نصوص القرآن مصدرها API alquran.cloud والتلاوة بصوت الشيخ مشاري العفاسي.
        </p>
      </main>
    </div>
  );
